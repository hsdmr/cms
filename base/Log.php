...
class Log
{
    ...
    {
        self::$currentJob = $job;
        $log = '[' . date('Y-m-d H:i:s') . '] ' . self::$currentJob . ' started.' . PHP_EOL;
        self::insert($log, 'daily');
    }
    
    public static function endJob()
    {
        $log = '[' . date('Y-m-d H:i:s') . '] ' . self::$currentJob . ' ended.' . PHP_EOL;
        self::insert($log, 'daily');
    }
    ...
    public static function error($response, $e, $th)
    {
        ...
        self::insert($log, 'daily');
        $log = '[' . date('Y-m-d H:i:s') . '] {"message": "' . $e->getMessage() . '", "status": "' . $e->http_code . '", "file": "' . $e->getFile() . '", "line": "' . $e->getLine() . '"}' . PHP_EOL . PHP_EOL;
